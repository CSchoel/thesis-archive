...
name.xsi <- paste(name, "000.xsi", sep="")
...
make.plots <- function(data, sub.title) {
	...
	plot(data$"t", data$"nu_b", type="l", col="red", xlim=lim, ylim=c(0, 3),
			main=paste( "Neuronal Activity for", name, sub.title, sep=" "),
	...
	plot(data$"t", data$"Rm", type="l", xlim=lim, col="purple",
	     main=paste("Mechanical respiratory Influence", name, sub.title, sep=" "),
	     xlab="time [s]", ylab="cos()")
